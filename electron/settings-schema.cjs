...
var SESSION_NAV_ACTIONS = /* @__PURE__ */ new Set(["previousSession", "nextSession"]);
...
function normalizeSessionNavBinding(action, binding, defaultBinding) {
...
function isBareEnterBinding(binding) {
  return keyMatches(binding.key, "Enter") && binding.command !== true && binding.ctrl !== true && binding.meta !== true && binding.alt !== true;
}
function normalizeComposerEnterBinding(action, binding, defaultBinding) {
  if (action !== "sendMessage" && action !== "insertNewline") {
    return binding ?? defaultBinding;
  }
  if (!binding) return { ...defaultBinding };
  if (action === "sendMessage" && isBareEnterBinding(binding) && binding.shift === void 0) {
    return { ...defaultBinding };
  }
  if (action === "insertNewline" && binding.shift === void 0 && isBareEnterBinding(binding)) {
    return { ...defaultBinding };
  }
  return binding;
}
function normalizeKeyboardShortcuts(saved) {
  ...
  for (const def of SHORTCUT_DEFINITIONS) {
    ...
    if (binding && typeof binding === "object" && typeof binding.key === "string") {
      ...
      const fallback = def.id === "toggleWebPanel" ? normalizeToggleWebPanelBinding(void 0, def.defaultBinding) : normalizeSessionNavBinding(def.id, void 0, def.defaultBinding);
...
function normalizeCometMindSettings(input, fallbackWorkspacePath = "") {
  ...
  return {
    ...
    gateway: {
      discord: {
        enabled: typeof discord.enabled === "boolean" ? discord.enabled : defaults.gateway.discord.enabled,
        botToken,
        botTokenEnv,
        ...
        requireMention: typeof discord.requireMention === "boolean" ? discord.requireMention : defaults.gateway.discord.requireMention,
        workspacePath: String(discord.workspacePath ?? defaults.gateway.discord.workspacePath).trim() || defaults.gateway.discord.workspacePath
      }
    }
  };
}
...
function normalizeCaretTrailSettings(settings) {
...
function normalizeProviders(providers) {
  ...
  const customProviders = saved.filter((provider) => !DEFAULT_PROVIDERS.some((p) => p.id === provider.id)).map((provider) => normalizeProvider(provider));
  return [...normalizedDefaults, ...customProviders];
}
function newProvider(id) {
  return {
    id,
    name: "New Provider",
    method: "openai-compatible",
    ...
    providers: [
      {
        ...
        method: id === "openai" && String(saved.baseURL || "").includes("opencode.ai") ? "opencode-go" : id === "openai" ? "openai-compatible" : id,
        enabled: true,
        baseURL: String(saved.baseURL || "").trim(),
        apiKey: String(saved.apiKey || "").trim(),
        selectedModel: String(saved.selectedModel || "").trim(),
        models: Array.isArray(saved.models) ? saved.models.map((m) => String(m || "").trim()).filter(Boolean) : [],
...
function runtimeProviders(settings) {
...
var providerSettingsSchema = external_exports.object({
  ...
  appearance: external_exports.object({
    ...
    caretTrail: external_exports.object({
      ...
      speed: external_exports.number().min(0).max(1)
    })
  }),
  ...
  cometmind: external_exports.object({
    ...
    gateway: external_exports.object({
      discord: external_exports.object({
        ...
        botTokenEnv: external_exports.string(),
        providerId: external_exports.string(),
        modelId: external_exports.string(),
        allowedUsers: external_exports.array(external_exports.string()),
        allowedChannels: external_exports.array(external_exports.string()),
...
function parseAndNormalizeSettings(raw, options = {}) {
  ...
  const partial = raw;
  const base = migrated ? { ...defaultSettings(), ...partial, ...migrated } : { ...defaultSettings(), ...partial };
